...
// Declare only once — make sure this isn't duplicated elsewhere
const coursesData = [
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function renderCourses(filteredCourses) {
    ...
    filteredCourses.forEach((course) => {
      ...
      courseCard.innerHTML = `
        ...
        ${
          course.completed
            ? `<p class="status" aria-label="Course completed">✅ Completed</p>`
            : ""
        }
      `;
  ...
  filterButtons.forEach((button) => {
    ...
    button.addEventListener("click", () => {
      ...
      const filtered = coursesData.filter((course) => course.subject === subject);
